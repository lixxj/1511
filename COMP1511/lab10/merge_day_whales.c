...
int same_day_and_species(struct pod *curr, struct pod *existing);
...
/* The function below will check if curr and existing are the same. 
If true, return 1, else return 0.*/
int same_day_and_species(struct pod *curr, struct pod *existing) {
    int result = 1;
    if (existing == NULL || curr == NULL) {
        return 0;
    }
    int a = strcmp(existing->species, curr->species);
    if (a != 0) {
        result = 0;
    }
    if (existing->when->day != curr->when->day) {
        result = 0;
    }
    if (existing->when->month != curr->when->month) {
        result = 0;
    }
    if (existing->when->year != curr->when->year) {
        result = 0;
    }
