...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    int start = start_day->day + 100 * start_day->month + 10000 * start_day->year;
    int end = finish_day->day + 100 * finish_day->month + 10000 * finish_day->year;
    
    while (first_pod != NULL) {
        int target = first_pod->when->day + 100 * first_pod->when->month + 10000 * first_pod->when->year;
        if (target >= start && target <= end) {
            printf("%02d/%02d/%02d ", first_pod->when->day, first_pod->when->month, first_pod->when->year);
            printf("%2d ", first_pod->how_many);
            printf("%s\n", first_pod->species);
        }
