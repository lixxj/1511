...
int how_many_same_tens(int *array, int size, int target);
...
void test_smallest_same_tens_digit(void);
void test_how_many_same_tens(void);
...
void choose_discards(void) {
    ...
    while (choose_index >= START) {
        ...
        if (initial_card[choose_index] == BUFFALO) {
            if (discard_func(initial_card) == FALSE) {
                discard[discard_index] = BUFFALO;
                discard_index++;
            } 
        }
    ...
    printf("%d %d %d\n", discard[START], 
           discard[START + ONE], discard[START + TWO]);
...
void choose_card_to_play(void) {
    ...
    if (num_of_cards_played_in_round == FALSE) {
        ...
        if (Have_same_tens(prev_cards, num_of_pre, CALF) == TRUE) {
            int candidate1 = 46;
            ...
            // oneX means the integers have tens digit "1", so as twoX
            if (sum_of_same_tens(prev_cards, num_of_pre, TWOX) 
               >= sum_of_same_tens(prev_cards, num_of_pre, ONEX) && 
               Have_same_tens(cards_in_hand, num_of_cards_in_hand, ONEX) 
                == TRUE) {
                // If have more than one same digit cards
                if (how_many_same_tens(cards_in_hand, 
                    num_of_cards_in_hand, ONEX) > ONE) {
                    
                    card[index_for_card] = 
                    second_smallest_same_tens_digit(cards_in_hand, 
                    num_of_cards_in_hand, ONEX);
                    index_for_card++;
                }
                ...
                if (how_many_same_tens(cards_in_hand, 
                    ...
                    (cards_in_hand, num_of_cards_in_hand, ONEX);
                    index_for_card++;
                }
            } else if (sum_of_same_tens(prev_cards, num_of_pre, TWOX) 
                       < sum_of_same_tens(prev_cards, num_of_pre, ONEX) 
                       && Have_same_tens(cards_in_hand, 
                       num_of_cards_in_hand, TWOX) == TRUE) {
                // If have more than one same digit cards
                if (how_many_same_tens(cards_in_hand, 
                    num_of_cards_in_hand, TWOX) > ONE) {
                    
                    card[index_for_card] = 
                    second_smallest_same_tens_digit(cards_in_hand, 
                    num_of_cards_in_hand, TWOX);
                    index_for_card++;
                }
                // Only have one same digit card
                if (how_many_same_tens(cards_in_hand, 
                    ...
                    card[index_for_card] = smallest_same_tens_digit
                    (cards_in_hand, num_of_cards_in_hand, TWOX);
                    index_for_card++;
                }
            } else {
                ...
                int oneX = ONEX;
                int twoX = TWOX;
                while (oneX >= ONEX && oneX <= 19 && twoX >= TWOX && 
                       twoX <= 29) {
            ...
            if (Have(cards_in_hand, num_of_cards_in_hand, BUFFALO) 
                == TRUE) {
                card[index_for_card] = BUFFALO;
                index_for_card++;
            }
        ...
        if (Have_same_tens(prev_cards, num_of_pre, CALF) == FALSE) {
            int candidate4 = 46;
               ...
               Have_same_tens(cards_in_hand, num_of_cards_in_hand, ONEX) 
                ...
                if (how_many_same_tens(cards_in_hand, 
                    ...
                    index_for_card++;
                       ...
                       ONEX) && Have_same_tens(cards_in_hand, 
                       num_of_cards_in_hand, TWOX) == TRUE) {
                // If have more than one same digit cards
                if (how_many_same_tens(cards_in_hand, 
                    num_of_cards_in_hand, TWOX) > ONE) {
            ...
            // if execute until here, means only have culves
            int candidate5 = 34;
            while (candidate5 <= 34 && candidate5 >= 30) {
                if (Have(cards_in_hand, num_of_cards_in_hand, 
                    candidate5) == TRUE) {
                    
                    card[index_for_card] = candidate5;
                    index_for_card++;
                }
                candidate5--;
            }
            int candidate6 = 35;
            while (candidate6 <= 35 && candidate6 >= 39) {
                if (Have(cards_in_hand, num_of_cards_in_hand, 
                    candidate6) == TRUE) {
                    
                    card[index_for_card] = candidate6;
                    index_for_card++;
                }
                candidate6++;
            }
    ...
    if (num_of_cards_played_in_round != FALSE) {
        ...
        if (Have_same_tens(cards_in_hand, num_of_cards_in_hand, 
            ...
            if (how_many_same_tens(cards_in_hand, 
                num_of_cards_in_hand, 
                cards_played_in_round[START]) > ONE) {
...
int second_smallest_same_tens_digit(int *array, int size, int target) {  
    ...
    int index2, min = same_tens_array[START], 
        second = same_tens_array[START + ONE];  
    if (min > second) {  
        min = same_tens_array[START + ONE];  
        second = same_tens_array[START];  
    }  
    for (index2 = TWO; index2 < len; ++index2) {  
...
void test_Have(void) {
    ...
    int size4 = 0;
    int array1[3] = {23, 24, 33};
    int array2[5] = {22, 24, 33, 42, BUFFALO};
    int array3[1] = {35};
    int array4[10] = {0};
